<template>
  <transition name="page_wrapper">
    <div class="page_wrapper">
      <div class="page57 page" v-show="pageStatus">
        <div class="page_content">
          <img class="page_bg" src="./page57.png" alt="">
          <div class="page28_start_wrapper">
            <div class="empty"></div>
            <div class="start_btn_wrapper">
              <div
                class="start_btn needclick"
                @click="goNextpage"
              >
                <img src="./start-btn-medium.gif" alt="">
              </div>
            </div>
          </div>
        </div>
      </div>
      <page58 v-if="!pageStatus"></page58>
    </div>
  </transition>
</template>

<script type="text/ecmascript-6">
  import {goToNext} from 'common/js/mixins'
  import page58 from 'components/page58/page58'

  export default {
    mixins: [goToNext],
    data() {
      return {
        guideNum: 0,
        canGoNext: false
      }
    },
    mounted() {
      setTimeout(() => {
        this.canGoNext = true
      }, 5000)
    },
    methods: {
      init() {
        this.guideTimer = setInterval(() => {
          this.guideNum++
        }, 1000)
      },
      goNextpage() {
        if (this.canGoNext) {
          this.init()
          this.goNext(58)
          // this.audioPlay('audio0')
        }
      }
    },
    watch: {
      guideNum(newValue) {
        if (newValue === 0) {
        } else if (newValue === 1) {
          document.querySelector('#audio_page57_2').play()
        } else if (newValue === 4) {
          document.querySelector('#audio_page57_3').play()
        } else if (newValue === 7) {
          document.querySelector('#audio_page57_4').play()
        } else if (newValue === 10) {
          document.querySelector('#audio_page57_5').play()
        } else if (newValue === 13) {
          document.querySelector('#audio_page57_6').play()
        } else if (newValue === 15) {
          clearInterval(this.guideTimer)
        }
      }
    },
    components: {
      page58
    }
  }
</script>

<style lang="scss" scoped rel="stylesheet/scss">
  @import "~common/scss/common.scss";

  .page28_start_wrapper {
    @extend %page_start_wrapper;
    .empty {
      flex: 320;
    }
    .start_btn_wrapper {
      flex: 280;
      .start_btn {
        margin-left: percent(290);
        width: percent(300);
        height: percent(140, 280);
        user-select: none;
        border-radius: 4%;
        img {
          width: 100%;
          height: 100%;
        }
      }
    }
  }

  @keyframes box-s {
    0% {
      box-shadow: 0 0 50px rgba(255, 248, 0, 1);
    }
    100% {
      box-shadow: 0 0 50px rgba(255, 248, 0, 0);
    }
  }

  @-webkit-keyframes box-s {
    0% {
      box-shadow: 0 0 50px rgba(255, 248, 0, 1);
    }
    100% {
      box-shadow: 0 0 50px rgba(255, 248, 0, 0);
    }
  }
</style>
